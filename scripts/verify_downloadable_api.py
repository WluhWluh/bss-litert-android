...
def verify(aar: Path, lock_path: Path) -> None:
    ...
    if aar.stat().st_size != expected["baseAarByteSize"]:
        raise ValueError("Explicit-loader base AAR size differs from the lock")
    if sha256_file(aar) != expected["baseAarSha256"]:
        raise ValueError("Explicit-loader base AAR SHA-256 differs from the lock")
    ...
    if len(classes) != expected["classesJarByteSize"]:
        raise ValueError("Explicit-loader classes JAR size differs from the lock")
    if sha256_bytes(classes) != expected["classesJarSha256"]:
        raise ValueError("Explicit-loader classes JAR SHA-256 differs from the lock")
