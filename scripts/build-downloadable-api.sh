...
packaging_python_identity="$(
  python3 -c \
    'import sys,zlib; print(".".join(map(str, sys.version_info[:3])) + "/" + zlib.ZLIB_RUNTIME_VERSION)'
)"
[[ "${packaging_python_identity}" == \
   "${PACKAGING_PYTHON_VERSION}/${PACKAGING_ZLIB_VERSION}" ]] || {
  echo "Expected packaging Python/zlib ${PACKAGING_PYTHON_VERSION}/${PACKAGING_ZLIB_VERSION}, got ${packaging_python_identity}." >&2
  exit 1
}
...
toolchain = re.compile(
    r"^external/(?:androidsdk|local_jdk|llvm18_linux_x86_64|python_[^/]+|"
    r"remotejdk[0-9]+_[^/]+|sysroot_linux_x86_64_glibc_2_27)/"
)
...
python3 "${repo_root}/scripts/package_api_aar.py" \
  ...
  --manifest "${source_dir}/litert/kotlin/src/downloadable/AndroidManifest.xml" \
  --output "${base_aar}"
